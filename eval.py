...
from stark_qa import load_model, load_qa, load_skb
...
def parse_args():
    ...
    parser.add_argument(
        "--model",
        default="VSS",
        choices=["BM25", "Colbertv2", "VSS", "MultiVSS", "LLMReranker"],
    )
    parser.add_argument(
        "--split",
        default="test",
        choices=["train", "val", "test", "test-0.1", "human_generated_eval"],
    )
...
if __name__ == "__main__":
    ...
    args.query_emb_dir = osp.join(
        ...
        args.dataset,
        args.emb_model,
        f"query{query_emb_surfix}{args.surfix}",
    )
    args.node_emb_dir = osp.join(
        args.emb_dir, args.dataset, args.emb_model, f"doc{args.surfix}"
    )
    args.chunk_emb_dir = osp.join(
        args.emb_dir, args.dataset, args.emb_model, f"chunk{args.surfix}"
    )
    ...
    if args.batch_size > 0 and args.model == "VSS":
        for batch_idx in tqdm(range(0, len(indices), args.batch_size or len(indices))):
            batch_indices = [
                idx
                for idx in indices[
                    batch_idx : min(batch_idx + args.batch_size, len(indices))
                ]
            ...
            results = model.evaluate_batch(
                pred_ids, pred, answer_ids, metrics=eval_metrics
            )

            for i, result in enumerate(results):
                result["idx"], result["query_id"] = batch_indices[i], query_ids[i]
                result["pred_rank"] = pred_ids[
                    torch.argsort(pred[:, i], descending=True)[: args.save_topk]
                ].tolist()
                eval_csv = pd.concat(
                    [eval_csv, pd.DataFrame([result])], ignore_index=True
                )
    else:
