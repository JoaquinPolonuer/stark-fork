...
from stark_qa.tools.local_encoder import (get_gritlm_embeddings,
                                          get_llm2vec_embeddings)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="llm2vec pdft model",
        metavar="ENCODE",
    )
    parser.add_argument(
        "--instruction", type=str, help="gritl/llm2vec instruction", metavar="ENCODE"
    )
    ...
    encode_kwargs = {
        ...
        if v is not None and parser._option_string_actions[f"--{k}"].metavar == "ENCODE"
    }
...
if __name__ == "__main__":
    ...
    mode_surfix = (
        "_human_generated_eval"
        if args.human_generated_eval and args.mode == "query"
        else ""
    )
    mode_surfix += "_no_rel" if not args.add_rel else ""
    mode_surfix += "_no_compact" if not args.compact else ""
    emb_dir = osp.join(
        args.emb_dir, args.dataset, args.emb_model, f"{args.mode}{mode_surfix}"
    )
    ...
    if args.mode == "query":
        qa_dataset = load_qa(
            args.dataset, human_generated_eval=args.human_generated_eval
        )
    ...
    if args.mode == "doc" and osp.exists(csv_cache):
        ...
        assert set(cache_dict.keys()) == set(
            lst
        ), "Indices in cache do not match the candidate indices."

        indices = list(set(lst) - set(exist_emb_indices))
        texts = [
            cache_dict[idx]
            for idx in tqdm(indices, desc="Filtering docs for new embeddings")
        ]
    else:
        indices = lst
        texts = [
            (
                qa_dataset.get_query_by_qid(idx)
                if args.mode == "query"
                else skb.get_doc_info(idx, add_rel=args.add_rel, compact=args.compact)
            )
