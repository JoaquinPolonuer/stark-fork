...
class Evaluator:
    ...
    def evaluate(
        ...
        all_pred = torch.ones(max(self.candidate_ids) + 1, dtype=torch.float) * (
            min(pred) - 1
        )
    ...
    def evaluate_batch(
        ...
        all_pred = torch.ones(
            (max(self.candidate_ids) + 1, pred.shape[1]), dtype=torch.float
        ) * (pred.min() - 1)
        all_pred[pred_ids, :] = pred
        all_pred = all_pred[self.candidate_ids].t().to(self.device)

        bool_gd = torch.zeros(
            (max(self.candidate_ids) + 1, pred.shape[1]), dtype=torch.bool
        )
        bool_gd[
            torch.concat(answer_ids),
            torch.repeat_interleave(
                torch.arange(len(answer_ids)), torch.tensor(list(map(len, answer_ids)))
            ),
        ] = True
        ...
        for i in range(len(answer_ids)):
            ...
            for metric in metrics:
                ...
                if metric == "mrr":
                    ...
                    result = retrieval_average_precision(
                        all_pred[i], bool_gd[i], top_k=k
                    )
                elif "ndcg" in metric:
