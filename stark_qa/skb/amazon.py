...
class AmazonSKB(SKB):
        ...
        **kwargs,
        ...
        if download_processed:
            if (self.root is None) or (
                ...
                sub_category_path = (
                    osp.join(self.root, "category_list.json")
    ...
    def get_doc_info(
        self, idx: int, add_rel: bool = False, compact: bool = False
    ) -> str:
        """
    ...
    def _process_raw(self, categories: list) -> dict:
        ...
        for category in review_categories:
            ...
            if not os.path.exists(osp.join(self.raw_data_dir, f"{category}.json.gz")):
                ...
                download_url(
                    f"{review_header}/categoryFiles/{category}.json.gz",
                    self.raw_data_dir,
                )
                download_url(
                    f"{review_header}/metaFiles2/meta_{category}.json.gz",
                    self.raw_data_dir,
                )
        for category in qa_categories:
            qa_header = RAW_DATA_HEADER["qa_header"]
            if not os.path.exists(
                osp.join(self.raw_data_dir, f"qa_{category}.json.gz")
            ):
    ...
    def post_process(
        self, raw_info: dict, meta_link_types: list, cache_path: str = None
    ) -> dict:
        """
            ...
            if link_type == "brand":
                ...
                for idx, node_info_i in enumerate(node_info.values()):
                    if link_type in node_info_i.keys():
                        ...
                        indice_list.extend(
                            [idx for _ in range(len(node_info_i[link_type]))]
                        )
    ...
    def construct_raw_node_info(
        self, df_meta: pd.DataFrame, df_review: pd.DataFrame, df_qa: pd.DataFrame
    ) -> dict:
        """
        ...
        def assign_colors(df_review, lower_limit=20):
                ...
                for key in style.keys():
                    if "color" in key.lower():
                        ...
                        raw_color_dict[asin].append(
                            style[key].strip().lower()
                            if isinstance(style[key], str)
                            else style[key][0].strip()
                        )
            ...
            color_counter = {
                k: v
                for k, v in sorted(
                    color_counter.items(), key=lambda item: item[1], reverse=True
                )
            }
            selected_colors = []
            for color, number in color_counter.items():
                if (
                    number > lower_limit
                    and len(color) > 2
                    and len(color.split(" ")) < 5
                    and color.isnumeric() is False
                ):
        ...
        filtered_color_dict_path = os.path.join(
            self.root, "intermediate", "filtered_color_dict.pkl"
        )
        ...
        for df_meta_i in tqdm(df_meta.itertuples()):
            ...
            for column in self.meta_columns:
                if column == "brand":
                    ...
                    category_list = [
                        category.lower()
                        for category in getattr(df_meta_i, column)
        ...
        for name, df, colunm_names in zip(
            ["review", "qa"], [df_review, df_qa], [self.review_columns, self.qa_columns]
        ):
        ...
        for df_i in df.itertuples():
            ...
            for edge_type_id, edge_type in enumerate(columns):
                if isinstance(getattr(df_i, edge_type), list):
                    in_nodes = [
                        self.asin2id[i]
                        for i in getattr(df_i, edge_type)
                        if i in self.asin2id
                    ]
