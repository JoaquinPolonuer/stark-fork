...
class MagSKB(SKB):
    ...
    def __init__(
        self, root: Union[str, None] = None, download_processed: bool = True, **kwargs
    ):
        """
        ...
        if download_processed:
            if (self.root is None) or (
                ...
                and not osp.exists(osp.join(self.root, "processed", "node_info.pkl"))
            ):
        ...
        if not osp.exists(self.schema_dir):
            download_hf_folder(
                ...
                DATASET["metadata"],
                repo_type="dataset",
                save_as_folder=self.schema_dir,
            )
        ...
        self.paper100M_text_cache_dir = osp.join(
            self.processed_data_dir, "paper100M_cache"
        )
        self.merged_filtered_path = osp.join(
            self.paper100M_text_cache_dir, "idx_title_abs.tsv"
        )
    ...
    def load_edge(self, edge_type: str) -> tuple:
        ...
        """
        edge_dir = osp.join(
            self.graph_data_root, f"raw/relations/{edge_type}/edge.csv.gz"
        )
        edge_type_dir = osp.join(
            self.graph_data_root, f"raw/relations/{edge_type}/edge_reltype.csv.gz"
        )
        num_dir = osp.join(
            self.graph_data_root, f"raw/relations/{edge_type}/num-edge-list.csv.gz"
        )
    ...
    def load_meta_data(self):
        ...
        if osp.exists(osp.join(self.mag_metadata_cache_dir, "paper_data.csv")):
            ...
            for t in ["author", "institution", "field_of_study", "paper"]:
                mag_csv[t] = pd.read_csv(
                    osp.join(self.mag_metadata_cache_dir, f"{t}_data.csv")
                )
            ...
            for key in reduced_attr.keys():
                ...
                if not osp.exists(file):
                    try:
                        download_url(
                            f'{RAW_DATA["mag_mapping"]}/{key}.txt.gz',
                            self.mag_mapping_dir,
                        )
                    except Exception as error:
                        print(
                            f'Download failed or {key} data not found, please download from {RAW_DATA["mag_mapping"]} to {file}'
                        )
                        raise error
                loaded_csv[key] = pd.read_csv(
                    file, header=None, sep="\t", usecols=column_nums
                )
                loaded_csv[key].columns = reduced_attr[key]

            print("Processing and merging meta data...")
            author_data = pd.read_csv(
                osp.join(self.ogbn_mag_mapping_dir, "author_entidx2name.csv.gz"),
                names=["id", "AuthorId"],
                skiprows=[0],
            )
            field_of_study_data = pd.read_csv(
                osp.join(
                    self.ogbn_mag_mapping_dir, "field_of_study_entidx2name.csv.gz"
                ),
                names=["id", "FieldOfStudyId"],
                skiprows=[0],
            )
            institution_data = pd.read_csv(
                osp.join(self.ogbn_mag_mapping_dir, "institution_entidx2name.csv.gz"),
                names=["id", "AffiliationId"],
                skiprows=[0],
            )
            paper_data = pd.read_csv(
                osp.join(self.ogbn_mag_mapping_dir, "paper_entidx2name.csv.gz"),
                names=["id", "PaperId"],
                skiprows=[0],
            )

            loaded_csv["Papers"].rename(
                columns={
                    "JournalId ": "JournalId",
                    "Rank": "PaperRank",
                    "CitationCount": "PaperCitationCount",
                },
                inplace=True,
            )
            loaded_csv["Journals"].rename(
                columns={
                    "DisplayName": "JournalDisplayName",
                    "Rank": "JournalRank",
                    "CitationCount": "JournalCitationCount",
                    "PaperCount": "JournalPaperCount",
                },
                inplace=True,
            )
            loaded_csv["ConferenceSeries"].rename(
                columns={
                    "DisplayName": "ConferenceSeriesDisplayName",
                    "Rank": "ConferenceSeriesRank",
                    "CitationCount": "ConferenceSeriesCitationCount",
                    "PaperCount": "ConferenceSeriesPaperCount",
                },
                inplace=True,
            )
            loaded_csv["ConferenceInstances"].rename(
                columns={
                    "DisplayName": "ConferenceInstancesDisplayName",
                    "CitationCount": "ConferenceInstanceCitationCount",
                    "PaperCount": "ConferenceInstancesPaperCount",
                },
                inplace=True,
            )

            author_data = author_data.merge(
                loaded_csv["Authors"], on="AuthorId", how="left"
            )
            field_of_study_data = field_of_study_data.merge(
                loaded_csv["FieldsOfStudy"], on="FieldOfStudyId", how="left"
            )
            institution_data = institution_data.merge(
                loaded_csv["Affiliations"], on="AffiliationId", how="left"
            )
            paper_data = paper_data.merge(
                loaded_csv["Papers"], on="PaperId", how="left"
            )

            paper_data["JournalId"] = (
                paper_data["JournalId"]
                .apply(lambda x: float(x))
                .apply(lambda x: -1 if np.isnan(x) else int(x))
            )
            paper_data = paper_data.merge(
                loaded_csv["Journals"], on="JournalId", how="left"
            )

            paper_data = paper_data.merge(
                loaded_csv["ConferenceSeries"], on="ConferenceSeriesId", how="left"
            )

            paper_data["ConferenceInstanceId"] = (
                paper_data["ConferenceInstanceId"]
                .apply(lambda x: float(x))
                .apply(lambda x: -1 if np.isnan(x) else int(x))
            )
            paper_data = paper_data.merge(
                loaded_csv["ConferenceInstances"], on="ConferenceInstanceId", how="left"
            )
            ...
            for csv_data in [
                ...
                for col in csv_data.columns:
                    csv_data[col] = csv_data[col].apply(
                        lambda x: -1 if isinstance(x, float) and np.isnan(x) else x
                    )
                    if (
                        "rank" in col.lower()
            ...
            for t in ["author", "institution", "field_of_study", "paper"]:
                mag_csv[t].to_csv(
                    osp.join(self.mag_metadata_cache_dir, f"{t}_data.csv"), index=False
                )
            author_data, paper_data = mag_csv["author"], mag_csv["paper"]
            field_of_study_data = mag_csv["field_of_study"]
            institution_data = mag_csv["institution"]
        ...
        institution_data.rename(
            columns={"id": "id", "AffiliationId": "mag_id"}, inplace=True
        )

        field_of_study_data["type"] = "field_of_study"
        field_of_study_data.rename(
            columns={"id": "id", "FieldOfStudyId": "mag_id"}, inplace=True
        )
    ...
    def load_english_paper_text(
        self, mag_ids: list, download_cache: bool = True
    ) -> pd.DataFrame:
        """
        ...
        if not osp.exists(self.merged_filtered_path):
            if download_cache:
                merged_filtered_zip_path = self.merged_filtered_path.replace(
                    "tsv", "zip"
                )
                download_hf_file(
                    DATASET["repo"],
                    DATASET["raw"],
                    repo_type="dataset",
                    save_as_file=merged_filtered_zip_path,
                )
                extract_zip(
                    merged_filtered_zip_path, osp.dirname(self.merged_filtered_path)
                )
            else:
                if not osp.exists(self.title_path):
                    raw_text_path = download_url(
                        RAW_DATA["ogbn_papers100M"], self.text_root
                    )
                ...
                title_abs_en = pd.merge(
                    title, abstract, how="outer", on="mag_id", sort=True
                )
                title_abs_en.to_csv(
                    self.merged_filtered_path, sep="\t", header=True, index=False
                )
    ...
    def get_doc_info(
        self, idx: int, compact: bool = False, add_rel: bool = False, n_rel: int = -1
    ) -> str:
        """
    ...
    def get_rel_info(
        self, idx: int, rel_types: Union[list, None] = None, n_rel: int = -1
    ) -> str:
        """
            ...
            for i in node_ids:
                if self[i].type == "paper":
                    ...
                    if str(self[i].DisplayName) != "-1":
                        institutions = self.get_neighbor_nodes(
                            i, "author___affiliated_with___institution"
                        )
                        ...
                        str_institutions = [
                            ...
                            if str(self[j].DisplayName) != "-1"
                        ]
                        if str_institutions:
                            str_institutions = ", ".join(str_institutions)
                            neighbors.append(
                                f"{self[i].DisplayName} ({str_institutions})"
                            )
                        else:
    ...
    def _process_raw(self):
        ...
        author_data, field_of_study_data, institution_data, paper_data = (
            self.load_meta_data()
        )
        ...
        author_id_to_mag = {
            row["id"]: row["mag_id"] for _, row in author_data.iterrows()
        }
        institution_id_to_mag = {
            row["id"]: row["mag_id"] for _, row in institution_data.iterrows()
        }
        field_of_study_id_to_mag = {
            row["id"]: row["mag_id"] for _, row in field_of_study_data.iterrows()
        }
        paper_mapping = pd.read_csv(
            osp.join(self.ogbn_mag_mapping_dir, "paper_entidx2name.csv.gz"),
            names=["id", "mag_id"],
            skiprows=[0],
        )
        ...
        for node_type, paper_rel in node_type_edge.items():
            ...
            if node_type == 3:
                ...
                mask = np.isin(edge_array[0], target_array) & np.isin(
                    edge_array[1], target_array
                )
                ...
                print(
                    f"{node_type} has {unique_paper_id.shape[0]} nodes left,  and {valid_edges_tensor.t().shape[0]} edges left."
                )
                continue
            else:
                edge = edge.t()
                ...
                print(
                    f"{node_type} has {other_ends.shape[0]} nodes left,  and {connected_edges.shape[0]} edges left."
                )
        ...
        print(
            f"1 has {other_ends.shape[0]} nodes left,  and {connected_edges.shape[0]} edges left."
        )
        ...
        id_to_mag = {}
        offset = 0
        node_type_overlapping_node_sort = {
            k: node_type_overlapping_node[k]
            for k in sorted(node_type_overlapping_node.keys())
        }
        ...
        for i, remain_node in node_type_overlapping_node_sort.items():
            old_to_new_mappings = {
                key: id + offset for id, key in enumerate(remain_node.tolist())
            }
            updated_dict = {
                value: domain_mappings[i][key]
                for key, value in old_to_new_mappings.items()
            }
        ...
        edges_final = torch.cat(
            [node_type_overlapping_edge[i] for i in range(4)], dim=1
        )
        ...
        field_of_study_data["new_id"] = field_of_study_data["id"].map(
            domain_old_to_new[2]
        )
