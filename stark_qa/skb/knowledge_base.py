...
class SKB:
    def __init__(
        ...
        **kwargs,
        ...
        self.candidate_ids = (
            self.get_candidate_ids()
            if hasattr(self, "candidate_types")
            else [i for i in range(len(self.node_info))]
        )
    ...
    def _build_sparse_adj(self):
        ...
        for edge_type in self.rel_type_lst():
            edge_idx = torch.arange(self.num_edges())[
                self.edge_types == self.edge_type2id(edge_type)
            ]
    ...
    def get_candidate_ids(self) -> list:
        ...
        candidate_ids = np.concatenate(
            [
                self.get_node_ids_by_type(candidate_type)
    ...
    def num_nodes(self, node_type_id=None):
        ...
        return (
            ...
            else sum(self.node_types == node_type_id)
        )
    ...
    def num_edges(self, node_type_id=None):
        ...
        return (
            len(self.edge_types)
            if node_type_id is None
            else sum(self.edge_types == node_type_id)
        )
    ...
    def get_node_ids_by_type(self, node_type: str) -> list:
        ...
        return torch.arange(self.num_nodes())[
            self.node_types == self.node_type2id(node_type)
        ].tolist()
    ...
    def get_edge_ids_by_type(self, edge_type: str) -> list:
        ...
        return torch.arange(self.num_edges())[
            self.edge_types == self.edge_type2id(edge_type)
        ].tolist()

    def sample_paths(
        self, node_types: list, edge_types: list, start_node_id=None, size=1
    ) -> list:
        """
    ...
    def get_all_paths(
        ...
        if len(neighbor_nodes) == 0:
            ...
            for iter_start_node_id in neighbor_nodes:
                subpaths = self.get_all_paths(
                    iter_start_node_id, node_types[1:], edge_types[1:]
                )
                if subpaths:
    ...
    def get_tuples(self) -> list:
        ...
        col_types, row_types = (
            self.node_types[col].tolist(),
            self.node_types[row].tolist(),
        )
        tuples_by_id = set(
            [(n_i, e, n_j) for n_i, e, n_j in zip(col_types, edge_types, row_types)]
        )
        tuples = [
            (self.node_type_dict[n_i], self.edge_type_dict[e], self.node_type_dict[n_j])
    ...
    def get_neighbor_nodes(self, idx, edge_type: str = "*") -> list:
        ...
        if edge_type == "*":
            ...
            neighbor_nodes = (
                self.sparse_adj_by_type[edge_type][idx]
                .coalesce()
                .indices()
                .view(-1)
                .tolist()
