...
def get_voyage_embedding(
    ...
    try:
        ...
        raise ImportError(
            "Please install the voyageai package using `pip install voyageai`."
        )
    ...
    for _ in range(max_retry):
        try:
            ...
            match = re.search(
                r"The max allowed tokens per submitted batch is (\d+). Your batch has (\d+) tokens after truncation",
                e,
            )
            if match is not None:
                ...
                for reduce_rate in range(9, 0, -1):
                    ...
                    try:
                        ...
                        print(
                            f"length={length} works! reduce_rate={0.1 * reduce_rate}."
                        )
                        return torch.FloatTensor(emb).view(1, -1)
                    except:
                        continue
...
def get_voyage_embeddings(
    ...
    assert all(
        [len(s) > 0 for s in texts]
    ), "every string in the `texts` list to be embedded should be non-empty"
