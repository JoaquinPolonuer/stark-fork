...
class VSS(ModelForSTaRKQA):
    ...
    def __init__(
        ...
        assert len(candidate_emb_dict) == len(
            self.candidate_ids
        ), "Mismatch in candidate embedding count."

        # Stack candidate embeddings into a tensor
        candidate_embs = [
            candidate_emb_dict[idx].view(1, -1) for idx in self.candidate_ids
        ]
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        query_emb = self.get_query_emb(
            query, query_id, emb_model=self.emb_model, **kwargs
        )

        # Compute similarity between query and candidate embeddings
        similarity = torch.matmul(
            query_emb.to(self.device).float(), self.candidate_embs.T.float()
        ).cpu()
